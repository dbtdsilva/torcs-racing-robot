# == Sensor list
# = State:   ['z', 'angle', 'gear', 'trackPos', 'speedY', 'distRaced', 'speedZ', 'damage', 'wheelSpinVel', 'focus',
#             'track', 'curLapTime', 'speedX', 'racePos', 'fuel', 'distFromStart', 'opponents', 'rpm', 'lastLapTime']
...
#       car within a range of 200 meters. When noisy option is enabled (see Section 7). Sensors are affected by i.i.d.
#       normal noises with a standard deviation equal to the 1% of sensors range. The sensors sample, with a resolution
#       of one degree, a five degree space along a specific direction provided by the client (the direction is defined
#       with the focus command and must be in the range [-90,+90] degrees w.r.t. the car axis). Focus sensors are not
#       always available: they can be used only once per second of simulated time. When the car is outside of the track
#       (i.e., pos is less than -1 or greater than 1), the focus direction is outside the allowed range ([-90,+90]
#       degrees) or the sensors has been already used once in the last second, the returned values are not reliable.
...
#       meters and returns the distance of the closest opponent in the covered area. When noisy option is enabled (see
#       Section 7), sensors are affected by i.i.d. normal noises with a standard deviation equal to the 2% of sensors
#       range. The 36 sensors cover all the space around the car, spanning clockwise from -180 degrees up to +180
#       degrees with respect to the car axis.
...
#       car within a range of 200 meters. When noisy option is enabled (see Section 7), sensors are affected by i.i.d.
#       normal noises with a standard deviation equal to the 10% of sensors range. By default, the sensors sample the
#       space in front of the car every 10 degrees, spanning clockwise from -90 degrees up to +90 degrees with respect
#       to the car axis. However, the con- figuration of the range finder sensors (i.e., the angle w.r.t. to the car
#       axis) can be set by the client once during initialization, i.e., before the beginning of each race. When the car
...
#       is 0 when car is on the axis, -1 when the car is on the right edge of the track and +1 when it is on the left
#       edge of the car. Values greater than 1 or smaller than -1 mean that the car is outside of the track.
